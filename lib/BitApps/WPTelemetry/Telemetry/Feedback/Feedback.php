...
namespace ScriptReport\Vendor\BitApps\WPTelemetry\Telemetry\Feedback;
...
class Feedback
{
    ...
    {
        return plugins_url( 'lib/BitApps/WPTelemetry/', SCRIPT_REPORT_FILE );
    }
    /**
    ...
    {
        Telemetry::view('deactivateModal', ['slug' => TelemetryConfig::getSlug(), 'prefix' => TelemetryConfig::getPrefix(), 'title' => TelemetryConfig::getTitle(), 'reasons' => $this->getDeactivateReasons()]);
    }
    public function getDeactivateReasons()
    {
        $reasons = ['found_a_better_plugin' => ['title' => esc_html__('Found a better plugin', TelemetryConfig::getSlug()), 'placeholder' => esc_html__('Which plugin?', TelemetryConfig::getSlug())], 'missing_specific_feature' => ['title' => esc_html__('Missing a specific feature', TelemetryConfig::getSlug()), 'placeholder' => esc_html__('Could you tell us more about that feature?', TelemetryConfig::getSlug())], 'not_working' => ['title' => esc_html__('Not working', TelemetryConfig::getSlug()), 'placeholder' => esc_html__('Could you tell us what is not working?', TelemetryConfig::getSlug())], 'not_working_as_expected' => ['title' => esc_html__('Not working as expected', TelemetryConfig::getSlug()), 'placeholder' => esc_html__('Could you tell us what do you expect?', TelemetryConfig::getSlug())], 'temporary_deactivation' => ['title' => esc_html__('It\'s a temporary deactivation', TelemetryConfig::getSlug()), 'placeholder' => ''], TelemetryConfig::getPrefix() . 'pro' => ['title' => esc_html__('I have ' . TelemetryConfig::getTitle() . ' Pro', TelemetryConfig::getSlug()), 'placeholder' => '', 'alert' => esc_html__('Wait! Don\'t deactivate ' . TelemetryConfig::getTitle() . '. You have to activate both ' . TelemetryConfig::getTitle() . ' and ' . TelemetryConfig::getTitle() . ' Pro in order to work the plugin.', TelemetryConfig::getSlug())], 'other' => ['title' => esc_html__('Other', TelemetryConfig::getSlug()), 'placeholder' => esc_html__('Please share the reason', TelemetryConfig::getSlug())]];
