...
namespace ScriptReport\Vendor\BitApps\WPTelemetry\Telemetry\Report;
...
class Report
{
    ...
    public function adminNotice()
    {
        ...
        Telemetry::view('reportOptIn', ['termsUrl' => TelemetryConfig::getTermsUrl(), 'policyUrl' => TelemetryConfig::getPolicyUrl(), 'optInUrl' => wp_nonce_url(add_query_arg(TelemetryConfig::getPrefix() . 'tracking_opt_in', 'true'), '_wpnonce'), 'optOutUrl' => wp_nonce_url(add_query_arg(TelemetryConfig::getPrefix() . 'tracking_opt_out', 'true'), '_wpnonce'), 'prefix' => TelemetryConfig::getPrefix(), 'title' => TelemetryConfig::getTitle(), 'description' => $this->getDescription()]);
    }
    public function getDescription()
    {
        return sprintf(
            // Translators: The user name and the plugin name.
            esc_html__('Hi, %1$s! This is an invitation to help our %2$s community.
				...
				And if you skip this, that’s okay! Plugin still work just fine.', '%3$s'),
    ...
    {
        if (!isset($_GET['_wpnonce']) || !wp_verify_nonce(sanitize_key($_GET['_wpnonce']), '_wpnonce') || !current_user_can('manage_options')) {
            return;
        }
    ...
    public function getTrackingData()
    {
        ...
        $data = ['url' => esc_url(home_url()), 'site' => $reportInfo->getSiteName(), 'admin_email' => get_option('admin_email'), 'first_name' => $user_name['firstName'], 'last_name' => $user_name['lastName'], 'server' => $reportInfo->getServerInfo(), 'wp' => $reportInfo->getWpInfo(), 'users' => $reportInfo->getUserCounts(), 'active_plugins' => \count($allPlugins['activePlugins']), 'inactive_plugins' => \count($allPlugins['inactivePlugins']), 'ip_address' => $reportInfo->getUserIpAddress(), 'plugin_slug' => TelemetryConfig::getPrefix(), 'plugin_version' => TelemetryConfig::getversion(), 'is_local' => $reportInfo->isLocalServer(), 'skipped' => false];
    ...
    {
        $collectList = ['Server environment details (php, mysql, server, WordPress versions)', 'Number of users in your site', 'Site language', 'Number of active and inactive plugins', 'Site name and URL', 'Your name and email address'];
        if ($this->addPluginData) {
    ...
    private function trackingSkippedRequest()
    {
        ...
        $data = ['url' => esc_url(home_url()), 'plugin_slug' => TelemetryConfig::getPrefix(), 'plugin_version' => TelemetryConfig::getversion(), 'skipped' => true, 'previously_skipped' => $previouslySkipped];
