...
class ReportInfo
{
    ...
    public function isLocalServer()
    {
        ...
        $isLocal = false;
        if (\in_array($ip, ['127.0.0.1', '::1'], true) || !strpos($host, '.') || \in_array(strrchr($host, '.'), ['.test', '.testing', '.local', '.localhost', '.localdomain'], true)) {
            $isLocal = true;
        }
        return $isLocal;
    }
    ...
    {
        global $wpdb;
        return (int) $wpdb->get_var($wpdb->prepare("SELECT count(ID) FROM {$wpdb->posts} WHERE post_type = %s and post_status = %s", [$postType, 'publish']));
    }
    /**
     * Get user name.
     *
    ...
    {
        $users = get_users(['role' => 'administrator', 'orderby' => 'ID', 'order' => 'ASC', 'number' => 1, 'paged' => 1]);
    ...
    public function getAllPlugins()
    {
        ...
        return ['activePlugins' => $activePlugins, 'inactivePlugins' => $plugins];
    }
    /**
     * Get plugin info
     *
    ...
    public function getPluginInfo($activePlugins, $slug)
    {
        ...
        foreach ($activePlugins as $prefix => $plugin) {
            ...
            $pluginInfo[$prefix] = ['name' => isset($plugin['name']) ? $plugin['name'] : '', 'version' => isset($plugin['version']) ? $plugin['version'] : ''];
        }
