<div class="bitapps-dm-wrapper" id="<?php 
echo esc_attr($args['slug']);
?>-bitapps-dm-wrapper">
  <div class="bitapps-dm-dialog">
    <div class="bitapps-dm-header">
      <span class="bitapps-dm-header-title">
        <?php 
echo esc_html__('Quick Feedback', esc_attr($args['slug']));
?>
      </span>
    ...
    <form class="bitapps-dm-form" method="post">
      <?php 
wp_nonce_field(esc_attr($args['prefix']) . 'nonce', '_ajax_nonce');
?>
      <input type="hidden" name="action" value="<?php 
echo esc_attr($args['prefix']) . 'deactivate_feedback';
?>" />

      <div class="bitapps-dm-form-caption">
        <?php 
echo esc_html__('If you have a moment, please let us know how "' . esc_attr($args['title']) . '" can improve.', esc_attr($args['slug']));
?>
      </div>
      <div class="bitapps-dm-form-body">
        <?php 
foreach ($args['reasons'] as $reasonKey => $reason) {
    ?>
          <div class="bitapps-dm-input-wrapper">
            <input id="<?php 
    echo esc_attr($args['slug']) . '-deactivate-feedback-' . esc_attr($reasonKey);
    ?>" class="bitapps-dm-input" type="radio" name="reason_key" value="<?php 
    echo esc_attr($reasonKey);
    ?>" required />
            <label for="<?php 
    echo esc_attr($args['slug']) . '-deactivate-feedback-' . esc_attr($reasonKey);
    ?>" class="bitapps-dm-label"><?php 
    echo esc_html($reason['title']);
    ?></label>
            <?php 
    if (!empty($reason['placeholder'])) {
        ?>
              <input class="bitapps-dm-feedback-text" type="text" name="reason_<?php 
        echo esc_attr($reasonKey);
        ?>" placeholder="<?php 
...
<script type="text/javascript">
  (function($) {
    const <?php 
echo esc_attr($args['prefix']) . 'AdminDialogApp';
?> = {
      cacheElements: function() {
        this.cache = {
          $deactivateLink: $('#the-list').find('[data-slug="<?php 
echo esc_attr($args['slug']);
?>"] span.deactivate a'),
          $dialogWrapper: $('#<?php 
echo esc_attr($args['slug']);
?>-bitapps-dm-wrapper'),
          $dialogDialog: $('#<?php 
echo esc_attr($args['slug']);
?>-bitapps-dm-wrapper').find('.bitapps-dm-dialog'),
          $dialogHeader: $('#<?php 
echo esc_attr($args['slug']);
?>-bitapps-dm-wrapper').find('.bitapps-dm-header'),
          $dialogForm: $('#<?php 
echo esc_attr($args['slug']);
?>-bitapps-dm-wrapper').find('.bitapps-dm-form'),
          $dialogSubmit: $('#<?php 
echo esc_attr($args['slug']);
?>-bitapps-dm-wrapper').find('.bitapps-dm-form-submit'),
          $dialogSkip: $('#<?php 
echo esc_attr($args['slug']);
?>-bitapps-dm-wrapper').find('.bitapps-dm-form-skip'),
          $dialogCloseBtn: $('#<?php 
echo esc_attr($args['slug']);
?>-bitapps-dm-wrapper').find('.bitapps-dm-close-svg'),
          $dialogOpen: false
        }
      },
      bindEvents: function() {
    ...
    $(function() {
      <?php 
echo esc_attr($args['prefix']) . 'AdminDialogApp.init()';
?>
    })
  })(jQuery);
</script>